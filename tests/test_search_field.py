...
class TestSearchField:
    ...
    def test_search_participant_by_lastname(
        self, user_project, lastname, status_code, participant
    ):
    ...
    @pytest.mark.parametrize(
        "speciality, status_code",
        pytest.param("QA", 200, id="Valid speciality"),
        pytest.param("Gamer", 400, id="Invalid speciality"),
    )
    def test_search_participant_by_speciality(
        self, user_project, speciality, status_code
    ):
